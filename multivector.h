...
struct MultiVector{
  ...
  MultiVector times(MultiVector rhs){
    ...
    //            zx*rhs.wxz + yz*rhs.wzy + wyx*rhs.xy + wxz*rhs.zx + wzy*rhs.yz + xyz*rhs.wxyz - wxyz*rhs.wzy;//missing -wxyz*xyz?
    float _w = s*rhs.w + w*rhs.s - x*rhs.wx - y*rhs.wy - z*rhs.wz + wx*rhs.x + wy*rhs.y + wz*rhs.z + xy*rhs.wyx +
    zx*rhs.wxz + yz*rhs.wzy + wyx*rhs.xy + wxz*rhs.zx + wzy*rhs.yz + xyz*rhs.wxyz - wxyz*rhs.xyz;
    ...
    float _wyx = s*rhs.wyx - w*rhs.xy + x*rhs.wy - y*rhs.wx + z*rhs.wxyz - wx*rhs.y + wy*rhs.x - xy*rhs.w + zx*rhs.wzy - yz*rhs.wxz + wyx*rhs.s - wzy*rhs.zx + wxz*rhs.yz - wxyz*rhs.z - wz*rhs.xyz + xyz*rhs.wz; //missing +wxz*xy?
    float _wxz = s*rhs.wxz - w*rhs.zx - x*rhs.wz + y*rhs.wxyz + z*rhs.wx + wx*rhs.z - wz*rhs.x - xy*rhs.wzy - zx*rhs.w + yz*rhs.wyx + wxz*rhs.s + wzy*rhs.xy - wyx*rhs.yz - wxyz*rhs.y - wy*rhs.xyz + xyz*rhs.wy; // <--- e02 * e123 = -e031, e123 * e02 = +e031
    float _wzy = s*rhs.wzy - w*rhs.yz + x*rhs.wxyz + y*rhs.wz - z*rhs.wy - wy*rhs.z + wz*rhs.y + xy*rhs.wxz - zx*rhs.wyx - yz*rhs.w + wzy*rhs.s - wxz*rhs.xy + wyx*rhs.zx - wxyz*rhs.x - wx*rhs.xyz + xyz*rhs.wx; // <--- e01 * e123 = -e023, e123 * e01 = +e023
  ...
  MultiVector transform(MultiVector m){
    ...
    // std::cout << "Input MV xyz: " << m.xyz << "\n";
    return self.times(m).times(self.reverse());
  }
